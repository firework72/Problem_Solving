...
int n;
vector<pair<ll, ll>> s(SIZE, {0, 0}), e(SIZE, {0, 0});
struct Node {
    int s;
    int e;
    long double dist;
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = n + 1; j <= 2 * n; j++) {
            node.push_back({i, j, hypotl(max(s[i].first, s[j].first) - min(s[i].first, s[j].first), max(s[i].second, s[j].second) - min(s[i].second, s[j].second))});
        }
    }
